  ...
  if (!passThru) {
    ...
    if (eventStatus & 0x02) { // error detected, what is it?
      ...
      if (eventStatus & 0x10) { // new acceleration data available
        sentral.newData(accelCount, EM7180_AX, accelX, accelY, accelZ, 0.000488);
      }
      if (eventStatus & 0x20) { // new gyro data available
        sentral.newData(gyroCount, EM7180_GX, gyroX, gyroY, gyroZ, 0.153);
      }
      if (eventStatus & 0x08) { // new mag data available
        sentral.newData(magCount, EM7180_MX, magX, magY, magZ, 0.305176);
      }
    ...
    if (delt_t > 500) { // update LCD once per half-second independent of read rate
      ...
      if (SerialDebug) {
        if(printOnce == 1){
          Serial.println("accel X, Y, Z \t \t gyro X, Y, Z \t \t Mag X, Y, Z");
          printOnce++;
        }
        Serial.print((int)1000 * accelX); Serial.print(", ");Serial.print((int)1000 * accelY); Serial.print(", ");Serial.print((int)1000 * accelZ); //print x,y,z accelerometer
        Serial.print("\t");Serial.print(gyroX, 2); Serial.print(", ");Serial.print(gyroY, 2); Serial.print(", ");Serial.print(gyroZ, 2); //print x,y,z gyroscope
        Serial.print("\t");Serial.print((int)magX); Serial.print(", ");Serial.print((int)magY); Serial.print(", ");Serial.print((int)magZ); //print x,y,z magnetometer
        Serial.print("\n");
      }
    }
    delay(1);
