...
const unsigned int Aes::KEY_BITS_128 = 128;
 ...
 */
int Aes::rijndaelSetupEncrypt(u32 *rk, const u8 *key, int keybits)
{
  int i = 0;
  u32 temp;
 ...
 * @return the number of rounds for the given cipher key size.
 */
int  Aes::rijndaelSetupDecrypt(u32 *rk, const u8 *key, int keybits)
{
  int nrounds, i, j;
  u32 temp;
...
void  Aes::rijndaelEncrypt(const u32 *rk, int nrounds, const u8 plaintext[16],
  u8 ciphertext[16])
{
...
void  Aes::rijndaelDecrypt(const u32 *rk, int nrounds, const u8 ciphertext[16],
  u8 plaintext[16])
{
  u32 s0, s1, s2, s3, t0, t1, t2, t3;
  #ifndef FULL_UNROLL
...
Aes::Aes(const String &password, unsigned int keybits){
	encryptRk = new unsigned long int[RKLENGTH(keybits)];
	decryptRk = new unsigned long int[RKLENGTH(keybits)];

	key = new unsigned char[KEYLENGTH(keybits)];
 	for (unsigned int i=0; i < KEYLENGTH(keybits); i++){
 	...
 	encryptRounds = rijndaelSetupEncrypt(encryptRk, key, keybits);
 	decryptRounds = rijndaelSetupDecrypt(decryptRk, key, keybits);
}
...
void Aes::decrypt(unsigned char *ciphertext, unsigned char *plaintext, int size){
	int rounds = size / 16;
	for(int i=0; i<rounds; i++){
		rijndaelDecrypt(decryptRk, decryptRounds, ciphertext, plaintext);
		plaintext += 16;
		ciphertext += 16;
	}
}
